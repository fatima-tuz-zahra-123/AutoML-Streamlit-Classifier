...
def generate_report(raw_df, clean_df, results_df, best_model_data, preprocessing_log=None):
    """
    Generates an interactive report with preview and PDF download.
    """
    ...
    # Generate insights once for both preview and PDF
    ...
    # --- REPORT PREVIEW (Embedded/Viewable) ---
    st.subheader("📄 Report Preview")
    st.info("Review the complete report below before downloading the PDF version.")
    
    with st.container():
        # Create a bordered preview area
        st.markdown("""
        <div style="border: 2px solid #2962FF; border-radius: 10px; padding: 20px; background-color: #fafafa;">
        """, unsafe_allow_html=True)
        
        # 1. Title Section
        st.markdown("### AutoML Classification Report")
        st.markdown("*Generated by AutoML Streamlit App*")
        st.markdown("*By Asma Imran and Fatima Tuz Zahra - NUST, Pakistan*")
        st.markdown("---")
        ...
        with col2:
            st.metric("Processed Rows", f"{clean_df.shape[0]:,}")
            st.metric("Processed Columns", clean_df.shape[1])
        
        # Transformation summary
        rows_diff = raw_df.shape[0] - clean_df.shape[0]
        cols_diff = raw_df.shape[1] - clean_df.shape[1]
        st.markdown(f"**Transformation Summary:** {rows_diff} rows and {cols_diff} columns were modified during preprocessing.")
        st.markdown("---")
        
        # 3. AI Analysis
        st.markdown("#### 2. AI Analysis & Insights")
        ...
        best_model_row = results_df.loc[results_df['F1 Score'].idxmax()]
        st.success(f"**Best Model:** {best_model_row['Model']} with F1 Score of {best_model_row['F1 Score']:.4f}")
        st.markdown("---")
        ...
        best_model_name = best_model_row['Model']
        conclusion_text = f"""
Based on the comprehensive evaluation of multiple machine learning models, the **{best_model_name}** 
demonstrated superior performance with the highest F1 Score of **{best_model_row['F1 Score']:.4f}** 
and accuracy of **{best_model_row['Accuracy']:.4f}**. 

This model achieved the best balance between precision and recall, making it the recommended choice 
for deployment in production environments. The preprocessing steps applied to the data helped improve 
model performance by addressing data quality issues and preparing features appropriately.
        """
